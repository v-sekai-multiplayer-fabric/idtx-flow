...
extern "C" IDTX_CORE_API idtx_avatar_t* idtx_core_import_avatar_from_vrm(const char* path)
{
    ...
    // Spring bones — VRMC_springBone lives as a JSON blob on
    // data->extensions. Full parse needs a JSON reader; for the MVP we
    // detect presence (via extensions_used) so the empty handles get
    // re-emitted on round trip rather than silently dropped.
    for (size_t e = 0; e < data->extensions_used_count; ++e) {
        if (data->extensions_used[e] != nullptr
            && std::strcmp(data->extensions_used[e], "VRMC_springBone") == 0) {
            // No-op: signals presence; full parse lands when the
            // JSON reader path is in.
            break;
        }
    }
